...
public class CardMoveChecks {
    ...
    public static void deckLevelChecks(Deck sourceDeck, int sourceCardIndex, Deck destinationDeck) throws MoveException {
        // TODO: Write implementation
        if (destinationDeck.getDeckType() == DeckType.STACK)
            throw new MoveException("Moving cards to stock is not allowed");
        if (destinationDeck.getDeckType() == sourceDeck.getDeckType())
            throw new MoveException("Move source and destination can't be the same");

//        if (sourceDeck.getDeckType() == DeckType.WASTE) && (destinationDeck.getDeckType() == DeckType.COLUMN || DeckType.STACK);

     //"Move source and destination can't be the same"
    ...
    static boolean opposingColor(Card card1, Card card2){
        // TODO: Write implementation
        if (redSuit(card1) && !redSuit(card2) || redSuit(card2) && !redSuit(card1)) {
            return true;
